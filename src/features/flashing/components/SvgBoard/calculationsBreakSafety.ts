import {
  BREAK_END_START_LINE_TYPE,
  LINE_TYPE,
  POINT_TYPE,
  TYPE_END_LINES,
  TYPE_END_LINES_BREAK,
} from '@models';
...
const changeTheTypeLine = (typeLine: TYPE_END_LINES): TYPE_END_LINES => {
  ...
  return (
    has1 ? typeLine.replace('1', '2') : typeLine.replace('2', '1')
  ) as TYPE_END_LINES;
};
export const calculateTypeLine = ({
  points,
  angle,
}: BREAK_END_START_LINE_TYPE): POINT_TYPE[] => {
...
export const getAngleForTheLine = (
  line: LINE_TYPE,
  typeLine: TYPE_END_LINES,
): number => {
  ...
  const dataAngles = anglesBreaks.find(
    angleB => angleB.default === angleAzimut.toString(),
  );
