...
export const useGetStores = (): UseQueryResult<STORE[], any> =>
  useQuery({
    queryFn: () => getStores(),
...
export const useGetAccountAndCompany = (): UseQueryResult<
  RESPONSE_COMPANY_ACCOUNT,
  any
> =>
  useQuery({
...
export const useGetSupplier = (): UseQueryResult<SUPPLIER, any> =>
  useQuery({
    queryFn: () => getSupplier(),
...
export const useGetOrderValidations = (): UseQueryResult<
  ORDER_VALIDATIONS_TYPES[],
  any
> =>
  useQuery({
    queryFn: () => getFieldOrdersValidation(),
    queryKey: [queryKey.get_order_validations],
  });
