...
export const getDataFlashing = createSelector(
  jobsSelector,
  ...
  (state, dataSelect): undefined | FLASHINGS_DATA => {
    ...
    return dataJob.flashings.find(
      flashing => flashing.id === dataSelect.idFlashing,
    );
  },
);
...
export const getOrderNumber = createSelector(
  ...
  (state, idJob) => {
    ...
    if (!dataJob) return undefined;
    return dataJob.orderData?.orderNumber;
  },
);
